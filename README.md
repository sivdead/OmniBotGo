...
> ## 🚧 Work in Progress 🚧
>
> **该项目仍在积极开发中 (WIP)，尚不稳定，无法保证正常运行。**
>
> 核心功能正在重构和完善，API 可能会频繁变更。请勿在生产环境中使用。
>
> 欢迎参与贡献，但请注意当前代码库的变动性。

# **OmniBotGo - 统一消息适配器网关**

基于 Go 的统一 IM 网关（个人/自用 MVP）：用**单一实例**对接企业微信、钉钉、公众号、飞书等，把入站消息中继到后端 Webhook/业务逻辑，并把响应发回平台。当前以跑通金路径为主，不是生产级「AI 全能中台」。
...
### 核心功能 (Phase 1 - ✅ 已完成)
- **多平台支持**：企业微信、钉钉、微信公众号、飞书
- **统一消息模型**：屏蔽平台差异，提供一致的消息格式
- **灵活路由**：基于规则的消息路由（Bot、平台、消息类型、关键词）
- **Webhook集成**：支持将消息转发到后端服务
- **完整日志**：记录连接状态和API调用日志

### 增强功能 (Phase 2 - ✅ 已完成)
- **丰富消息类型**：支持文本、图片、音频、视频、文件、位置、链接、Markdown、卡片、图文等10+种消息类型
- **事件处理**：支持用户关注/取关、入群/退群、菜单点击等平台事件
- **钉钉企业应用**：增强钉钉适配器，支持企业内部应用的工作通知、群消息等功能
- **环境变量配置**：支持通过环境变量覆盖配置，便于容器化部署
- **状态监控API**：提供系统状态、平台连接、消息统计等监控端点

### 高级功能 (Phase 3 - 📋 规划中 / 未完成)
> 以下能力多数仍停在实体/CRUD 或文档层，**尚无可用的 worker / 调度 / 中间件落地**。当作 backlog，不要按已完成理解。
- **消息队列**：实体与管理 API 有雏形；真正的出队 worker / 重试调度尚未实现
- **消息去重**：设计文档有，运行时去重中间件未落地
- **插件 / 中间件系统**：未实现
- **配置热加载**：未实现
- **更多平台**（Slack、Teams 等）：仅规划

### 平台特性
- **企业微信**：支持文本、图片、文件、Markdown、图文消息，支持事件回调
- **钉钉**：
  - Stream模式：支持机器人实时消息接收
  - 企业应用模式：支持工作通知、群消息、事件回调
- **微信公众号**：支持各类消息和事件处理
- **飞书**：消息发送 + Webhook 收件（challenge/事件解析已补，持续完善中）
...
- **可运维 MVP**：先跑通单平台金路径（收发 + webhook/stream），再谈高并发与 7×24
- **可扩展性**：支持轻松添加新的平台适配器
...
- **钉钉 (DingTalk)**：Stream 机器人 + 企业内部应用
- **微信公众号**：订阅号和服务号消息处理
- **飞书 (Feishu)**：发送 + Webhook 收件（持续完善）
- **可扩展架构**：预留接口支持 Telegram、Slack 等
