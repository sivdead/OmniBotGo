...
// CreateChannel 创建通道
// @Summary 创建新的消息通道
// @Description 为指定平台创建新的消息通道配置
// @Tags channels
// @Accept json
// @Produce json
// @Param request body usecase.CreateChannelRequest true "创建通道请求"
// @Success 201 {object} StandardResponse{data=usecase.CreateChannelResponse} "创建成功"
...
// @Accept json
// @Produce json
// @Param bot_id query string false "机器人ID"
// @Param platform query string false "平台类型" Enums(wecom,dingtalk,wechat,feishu)
...
// @Success 200 {object} StandardResponse{data=usecase.ListChannelsResponse} "获取成功"
// @Failure 400 {object} StandardResponse "请求参数错误"
// @Failure 500 {object} StandardResponse "内部服务器错误"
// @Router /api/v1/channels [get]
func (v *V1) GetChannels(c *fiber.Ctx) error {
	var req usecase.ListChannelsRequest
...
// @Success 200 {object} StandardResponse{data=usecase.GetChannelByIDResponse} "获取成功"
// @Failure 400 {object} StandardResponse "请求参数错误"
...
func (v *V1) GetChannelByID(c *fiber.Ctx) error {
	...
	result, err := v.channelUC.GetChannelByID(c.Context(), &usecase.GetChannelByIDRequest{
		ID: id,
	})
	if err != nil {
...
// @Success 200 {object} StandardResponse{data=usecase.UpdateChannelResponse} "更新成功"
// @Failure 400 {object} StandardResponse "请求参数错误"
// @Failure 404 {object} StandardResponse "通道不存在"
...
func (v *V1) DeleteChannel(c *fiber.Ctx) error {
	...
	err := v.channelUC.DeleteChannel(c.Context(), &usecase.DeleteChannelRequest{
		ID: id,
	})
	if err != nil {
...
// @Produce json
// @Param id path string true "通道ID"
// @Param request body usecase.UpdateChannelStatusRequest true "更新状态请求"
// @Success 200 {object} StandardResponse{data=usecase.UpdateChannelStatusResponse} "更新成功"
// @Failure 400 {object} StandardResponse "请求参数错误"
// @Failure 404 {object} StandardResponse "通道不存在"
// @Failure 500 {object} StandardResponse "内部服务器错误"
...
func (v *V1) UpdateChannelStatus(c *fiber.Ctx) error {
	...
	var req usecase.UpdateChannelStatusRequest
	if err := c.BodyParser(&req); err != nil {
		v.l.Error("解析请求体失败: %v", err)
		return NewErrorResponse(c, http.StatusBadRequest, "请求参数格式错误")
	}
	...
	result, err := v.channelUC.UpdateChannelStatus(c.Context(), &req)
	if err != nil {
	...
	v.l.Info("通道状态更新成功: %s -> %s", id, req.Status)

	return NewSuccessResponse(c, http.StatusOK, "状态更新成功", result)
}
...
// @Produce json
// @Param id path string true "通道ID"
// @Success 200 {object} StandardResponse{data=usecase.RefreshChannelTokenResponse} "刷新成功"
// @Failure 400 {object} StandardResponse "请求参数错误"
// @Failure 404 {object} StandardResponse "通道不存在"
// @Failure 500 {object} StandardResponse "内部服务器错误"
...
func (v *V1) RefreshChannelToken(c *fiber.Ctx) error {
	...
	result, err := v.channelUC.RefreshChannelToken(c.Context(), &usecase.RefreshChannelTokenRequest{
		ID: id,
	})
	if err != nil {
	...
	return NewSuccessResponse(c, http.StatusOK, "令牌刷新成功", result)
}
