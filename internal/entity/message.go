...
func (m *Message) IsProcessed() bool {
	return m.MessageStatus.IsProcessed()
}

// CanRetry 检查消息是否可以重试
func (m *Message) CanRetry() bool {
	return m.MessageStatus.CanRetry()
}

// MarkAsProcessing 标记消息为处理中
func (m *Message) MarkAsProcessing() {
	m.MessageStatus = MessageStatusProcessing
}

// MarkAsProcessed 标记消息为已处理
func (m *Message) MarkAsProcessed() {
	m.MessageStatus = MessageStatusProcessed
	now := time.Now()
	m.ProcessedAt = &now
}
...
func (m *Message) MarkAsSent() {
	m.MessageStatus = MessageStatusSent
	now := time.Now()
	m.SentAt = &now
}
...
func (m *Message) MarkAsFailed(errorMsg string) {
	m.MessageStatus = MessageStatusFailed
...
func (m *Message) GetRawContentValue(key string) interface{} {
	if m.RawContent == nil {
		return nil
	}
...
func (m *Message) GetUnifiedContentValue(key string) interface{} {
...
func (m *Message) Validate() error {
	...
	if m.MessageType == "" {
		return NewValidationError("message_type", "消息类型不能为空")
	}
	return nil
}
