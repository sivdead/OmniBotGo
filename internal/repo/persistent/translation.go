...
// New -.
func New(db database.CommonDB) *TranslationRepo {
...
func (r *TranslationRepo) GetHistory(ctx context.Context) ([]entity.Translation, error) {
	...
	err := r.db.GetGORM().WithContext(ctx).Find(&entities).Error
	if err != nil {
		return nil, fmt.Errorf("TranslationRepo - GetHistory - r.db.GetGORM.Find: %w", err)
	}
...
func (r *TranslationRepo) Store(ctx context.Context, t entity.Translation) error {
	err := r.db.GetGORM().WithContext(ctx).Create(&t).Error
	if err != nil {
		return fmt.Errorf("TranslationRepo - Store - r.db.GetGORM.Create: %w", err)
	}
...
func (r *TranslationRepo) GetHistoryWithComplexConditions(ctx context.Context, conditions map[string]interface{}, limit, offset int, orderBy string) ([]entity.Translation, error) {
	...
	rows, err := r.db.GetSqlDB().QueryContext(ctx, sql, args...)
	if err != nil {
		return nil, fmt.Errorf("TranslationRepo - GetHistoryWithComplexConditions - r.db.GetSqlDB.QueryContext: %w", err)
	}
	defer rows.Close()
...
func (r *TranslationRepo) GetTranslationStatistics(ctx context.Context) ([]map[string]interface{}, error) {
	...
	rows, err := r.db.GetSqlDB().QueryContext(ctx, sql, args...)
	if err != nil {
		return nil, fmt.Errorf("TranslationRepo - GetTranslationStatistics - r.db.GetSqlDB.QueryContext: %w", err)
	}
	defer rows.Close()
...
func (r *TranslationRepo) BatchInsertTranslations(ctx context.Context, translations []entity.Translation) error {
	err := r.db.GetGORM().WithContext(ctx).CreateInBatches(translations, 100).Error
	if err != nil {
		return fmt.Errorf("TranslationRepo - BatchInsertTranslations - r.db.GetGORM.CreateInBatches: %w", err)
	}
...
func (r *TranslationRepo) UpdateTranslationsWithCondition(ctx context.Context, updateFields map[string]interface{}, whereConditions map[string]interface{}) error {
	query := r.db.GetSquirrel().Update("translations")
	...
	_, err = r.db.GetSqlDB().ExecContext(ctx, sql, args...)
	if err != nil {
		return fmt.Errorf("TranslationRepo - UpdateTranslationsWithCondition - r.db.GetSqlDB.ExecContext: %w", err)
	}
