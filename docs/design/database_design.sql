...
('server.port', '8080', 'int', 'server', '服务器端口'),
('server.host', '0.0.0.0', 'string', 'server', '服务器绑定地址'),
('log.level', 'info', 'string', 'logging', '日志级别'),
('message.max_size', '10485760', 'int', 'message', '最大消息大小(字节)'),
('webhook.timeout', '30', 'int', 'webhook', 'Webhook超时时间(秒)'),
('token.default_expire', '7200', 'int', 'auth', '默认Token过期时间(秒)'),
('message.retention_days', '30', 'int', 'message', '消息保留天数'),
('queue.max_size', '10000', 'int', 'queue', '队列最大长度'),
('webhook.base_url', 'http://localhost:8080', 'string', 'webhook', 'Webhook基础URL'),
('channel.webhook_path_template', '/webhook/channel-{id}', 'string', 'channel', '通道Webhook路径模板');

-- 插入示例通道配置
INSERT INTO channels (bot_id, platform_type, channel_name, webhook_path, config) VALUES
(1, 'wecom', '生产环境-企业微信', '/webhook/channel-1', '{"corp_id": "wwxxxxx", "app_id": "1000001", "app_secret": "xxxxx", "agent_id": "1000001"}'),
(1, 'dingtalk', '生产环境-钉钉', '/webhook/channel-2', '{"app_id": "dingxxxxx", "app_secret": "xxxxx"}'),
(2, 'wechat_official', 'AI助手-微信公众号', '/webhook/channel-3', '{"app_id": "wxxxxxx", "app_secret": "xxxxx"}');

-- 插入示例消息处理器
INSERT INTO message_processors (processor_name, processor_type, config) VALUES
('OpenAI聊天处理器', 'openai_chat', '{"api_key": "sk-xxxxx", "model": "gpt-3.5-turbo", "temperature": 0.7, "base_url": "https://api.openai.com"}'),
('客服工单系统', 'webhook_forwarder', '{"webhook_url": "https://crm.company.com/api/webhook", "api_key": "crm_api_key", "timeout": 30}'),
('订单查询处理器', 'order_query', '{"api_base_url": "https://order.company.com/api", "api_key": "order_api_key"}'),
('邮件通知处理器', 'email_notifier', '{"smtp_host": "mail.company.com", "smtp_user": "bot@company.com", "smtp_pass": "xxxxx"}'); 